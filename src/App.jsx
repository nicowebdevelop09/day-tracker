...
function loadData() {
  try {
    ...
    if (!raw) return { entries: {}, reminders: [], hiddenBase: [], customCategories: [], water: {}, waterGoal: 2000 };
    const parsed = JSON.parse(raw);
    return { entries: {}, reminders: [], hiddenBase: [], customCategories: [], water: {}, waterGoal: 2000, ...parsed };
  } catch {
    return { entries: {}, reminders: [], hiddenBase: [], customCategories: [], water: {}, waterGoal: 2000 };
  }
}
  ...
  return (
    <Tag_ onClick={onClick} className={onClick ? "dt-water-ring-btn" : undefined} style={{ position: "relative", width: size, height: size }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        <b style={{ fontSize: size > 56 ? 13 : 11 }}>{Math.round(pct * 100)}%</b>
...
function TodayTab({ data, setData }) {
  ...
  const [goalInput, setGoalInput] = useState(String(data.waterGoal));
  ...
  const saveGoal = () => {
    ...
    if (g && g > 0) setData((d) => ({ ...d, waterGoal: g }));
      ...
      {waterOpen && (
        <div className="dt-water-panel">
          ...
          <div className="dt-card" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            <div className="dt-water-panel-header">
              {goalEditing ? (
                <div className="dt-water-goal-edit">
                  <input
                    type="number" value={goalInput} onChange={(e) => setGoalInput(e.target.value)}
                    style={{ background: PAPER, border: `1px solid ${PAPER_LINE}`, color: INK, borderRadius: 8, padding: "6px 8px", width: 90, fontSize: 13 }}
                  />
                  <span style={{ color: MUTED, fontSize: 12 }}>ml</span>
                  <button onClick={saveGoal} className="dt-link-btn"><Check size={14} /></button>
                  <button onClick={() => setGoalEditing(false)} className="dt-link-btn"><X size={14} /></button>
                </div>
              ) : (
                <>
                  <span style={{ color: MUTED, fontSize: 13 }}>Obiettivo: {formatWater(data.waterGoal)}</span>
                  <button onClick={() => { setGoalInput(String(data.waterGoal)); setGoalEditing(true); }} className="dt-link-btn">
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <input
                type="number" inputMode="numeric" value={waterInput} onChange={(e) => setWaterInput(e.target.value)}
                placeholder="ml"
                style={{ flex: 1, background: PAPER, border: `1px solid ${PAPER_LINE}`, color: INK, borderRadius: 10, padding: "10px 12px", fontSize: 15 }}
              />
              <button onClick={addWater} className="dt-btn-primary" style={{ width: "auto", padding: "10px 18px" }}>
