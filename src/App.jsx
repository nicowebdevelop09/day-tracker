...
function loadData() {
  try {
    ...
    if (!raw) return { entries: {}, reminders: [], hiddenBase: [], customCategories: [], water: {}, waterGoal: 2000, allowOnetime: true, waterUnit: "ml", profile: DEFAULT_PROFILE, onboarded: false };
    const parsed = JSON.parse(raw);
    return { entries: {}, reminders: [], hiddenBase: [], customCategories: [], water: {}, waterGoal: 2000, allowOnetime: true, waterUnit: "ml", profile: DEFAULT_PROFILE, onboarded: false, ...parsed };
  } catch {
    return { entries: {}, reminders: [], hiddenBase: [], customCategories: [], water: {}, waterGoal: 2000, allowOnetime: true, waterUnit: "ml", profile: DEFAULT_PROFILE, onboarded: false };
  }
}
...
function Onboarding({ setData }) {
  ...
  const submit = () => {
    ...
    setData((d) => ({
      ...
      waterGoal: goal ? parseInt(goal, 10) : d.waterGoal,
      onboarded: true,
    }));
  };
  ...
  return (
    <div className="dt-app">
      ...
      <div className="dt-shell" style={{ justifyContent: "center", padding: "40px 24px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div className="dt-field">
            <label>Obiettivo acqua giornaliero (ml)</label>
            <input type="number" inputMode="numeric" value={goal} onChange={(e) => setGoal(e.target.value)} />
          </div>
          {error && <p className="dt-error">{error}</p>}
          <button onClick={submit} className="dt-btn-primary" style={{ marginTop: 8 }}>
            Inizia
          </button>
        </div>
      </div>
...
export default function App() {
  ...
  if (!data.onboarded) {
    return <Onboarding setData={setData} />;
  }
